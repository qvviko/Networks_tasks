...
int hash_function(struct map *hashmap, void *key) {
...
int init_map(struct map *hashmap, size_t key, size_t value, int max_size) {
    hashmap->length = 0;
...
int add_item(struct map *hashmap, void *key, void *value) {
...
void *remove_item(struct map *hashmap, void *key) {
    int index = hash_function(hashmap, key);
    struct MapNode *node = hashmap->inner_array[index];
...
void *find(struct map *hashmap, void *key) {
